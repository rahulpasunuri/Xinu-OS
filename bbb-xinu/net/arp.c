	...
	)
{
	...
	if (found) {
		...
		if (arptr->arstate == AR_PENDING) {
			...
			arptr->entryTime = clktime; //set the entry time for the ARP entry..
			send(arptr->arpid, OK);
		}
	}
...
/*
Deletes all the stale ARP entries from the cache..
*/
void deleteStaleEntries()
{
    ...
    for(i=0; i<ARP_SIZ; i++)
    {
	    if (arpcache[i].arstate == AR_RESOLVED) {
	        if ((clktime - arpcache[i].entryTime) > ARP_CACHE_TIMEOUT)
	        {
	            //delete the entry..(just updating the state is enough...)
	            arpcache[i].arstate = AR_FREE;
	            kprintf("Stale ARP Entry cleared!!");
	        }
	    }
    }
